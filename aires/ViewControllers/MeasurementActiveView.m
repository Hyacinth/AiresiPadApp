...
@interface MeasurementActiveView ()
{
...
@implementation MeasurementActiveView
...
-(IBAction)donePressed:(id)sender
{
    if(_editMode)
    {
        if(_delegate && [_delegate respondsToSelector:@selector(measurementActiveDonePressed:)])
        {
            [self endEditing:YES];
            ...
            [_delegate measurementActiveDonePressed:_sampleMeasurement];
        }
    }
    else
    {
        if(_delegate && [_delegate respondsToSelector:@selector(measurementActiveAddPressed:)])
        {
            [self endEditing:YES];
            [[NSNotificationCenter defaultCenter] removeObserver:self];
            ...
            [_delegate measurementActiveAddPressed:_measurementFields];
        }
    }
}
...
{
    if(_delegate && [_delegate respondsToSelector:@selector(measurementActiveCancelPressed)])
    {
        [self endEditing:YES];
        [[NSNotificationCenter defaultCenter] removeObserver:self];
        [_delegate measurementActiveCancelPressed];
    }
}

-(IBAction)deletePressed:(id)sender
{
    if(_delegate && [_delegate respondsToSelector:@selector(measurementActiveDeletePressed:)])
    {
        [self endEditing:YES];
        [[NSNotificationCenter defaultCenter] removeObserver:self];
        [_delegate measurementActiveDeletePressed:_sampleMeasurement];
    }
}
